...
def u_sol(x1, x2, t):
   op = 1 + 4 * (t)
   return (x1.square() + x2.square()) / op + torch.log(op)
...
def compute_pde_loss(N, x1, x2, t, create_graph=True):
  ...
  return (u_t - (u_x1x1 + u_x2x2) + u_x1**2 + u_x2**2).square().mean()
...
for i in range(num_sig):
  ...
  torch.save(net_dict, f"weights/HJB_dNs_{d_Ns}_s_{sigmas[i]}_i_{iterations}.pt")
  np.savez(f"results/HJB_dNs_{d_Ns}_s_{sigmas[i]}_i_{iterations}",train=train_errors,test=test_errors)